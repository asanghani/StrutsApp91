...
*  
**************** Tutorial - 15 Using Model objects(M-V-C)- practicle-part -1
*	-->util right now we seen we passing parameter induvidual not in package.let's say we have 100 feild in form.
*   -->Modle object - model object is package which use to send information( viewform data) between to layer. view to backend.
...
*   -->http://localhost:8080/StrutsApp/login.jsp
*   -->until right now we passing iduvidual filed value from .jsp to action and than setting that induvidual value in user object
